                ...
                <div class="mt-4 px-1 mb-4 flex items-center justify-between">
                    ...
                    <div class="flex gap-5 items-center">
                        ...
                        <a href="{{ route('post.show', ['user' => $post->user, 'post' => $post]) }}"
                            ...
                            <span class="text-sm text-gray-500">{{ $post->claps_count }}</span>
                        </a>
                        <a href="{{ route('post.show', ['user' => $post->user, 'post' => $post]) }}"
                            class="flex gap-1 items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                    ...
                    <div x-data="{ open: false }" class="relative inline-block text-left"> <!-- Trigger button -->
                        <div @click="open = !open" class="text-gray-500 cursor-pointer"> <svg
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            @auth @if (auth()->user()->id === $post->user->id)
                                ...
                                <form action="{{ route('post.delete', $post) }}" method="POST"> @csrf
                                    @method('DELETE') <button type="submit"
                                        ...
                                        Delete </button> </form>
                            @else
                                <!-- Follow/Unfollow --> <x-follow-container :user="$post->user"> <a href="#"
                            ...
                            @endif
                        @else
                            <!-- For guest users, only show Follow --> <x-follow-container :user="$post->user"> <a
